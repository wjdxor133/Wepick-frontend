                ...
                <ChoiceBox>
                    {
                        filter && filter.map((region, idx) => {
                            return <li onClick={() => setFilterRegion(region)} style={{ backgroundColor: filterRegion === region.name ? '#2986FA' : '#F8F8FA', color: filterRegion === region.name ? 'white' : '#333333' }}>{region.name}</li>
                        })
                    }
                </ChoiceBox>
            </Area>
